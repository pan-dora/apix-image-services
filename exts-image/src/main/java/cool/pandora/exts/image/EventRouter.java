...
package cool.pandora.exts.image;
...
public class EventRouter extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("direct:get")
                ...
                .when(and(header(CONTENT_TYPE).startsWith("image/"),
                        header("Link").contains("<http://www.w3.org/ns/ldp#NonRDFSource>;"
                                + "rel=\"type\"")))
                .log(INFO, LOGGER, "Image Processing ${headers[CamelHttpPath]}")
                .to("direct:convert")
                .when(header("Link").contains("<http://www.w3.org/ns/ldp#NonRDFSource>;"
                        + "rel=\"type\""))
    ...
    private static String cmdOptions(final Exchange e) {
